...
class TestPrompts:
    ...
    def test_load_prompts_from_project(self, tmp_path):
        """Project .worker/prompts/*.md are loaded."""
        from worker_core.prompts import load_prompts

        prompts_dir = tmp_path / ".worker" / "prompts"
    ...
    def test_load_prompts_global(self, tmp_path, monkeypatch):
        ...
        fake_config.mkdir()
        monkeypatch.setattr(prompts_mod, "CONFIG_DIR", fake_config)
    ...
    def test_project_overrides_global(self, tmp_path, monkeypatch):
        ...
        project_dir.mkdir()
        project_prompts = project_dir / ".worker" / "prompts"
...
class TestSkills:
    ...
    def test_load_skills_with_frontmatter(self, tmp_path):
        ...
        skills_dir = tmp_path / ".worker" / "skills"
    ...
    def test_project_skills_override_global(self, tmp_path, monkeypatch):
        ...
        fake_config = tmp_path / "config"
        fake_config.mkdir()
        monkeypatch.setattr(skills_mod, "CONFIG_DIR", fake_config)
        ...
        project_dir = tmp_path / "project"
        project_dir.mkdir()
        project_skills = project_dir / ".worker" / "skills"
...
class TestThemes:
    ...
    def test_project_theme_override(self, tmp_path, monkeypatch):
        ...
        project_dir = tmp_path / "project"
        project_dir.mkdir()
        project_themes = project_dir / ".worker" / "themes"
