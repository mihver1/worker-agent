...
LEGACY_CONFIG_DIR_ENV = CONFIG_DIR_ENV
...
OFFICIAL_REGISTRY_URL = (
    "https://raw.githubusercontent.com/mihver1/artel/main/registry/extensions.toml"
)
...
def load_config(project_dir: str | None = None) -> ArtelConfig:
    ...
    if global_config.exists():
        with open(global_config, "rb") as f:
            data = tomllib.load(f)
        config = ArtelConfig.model_validate(data)
    ...
    if project_dir:
        ...
        if project_source.exists():
            ...
            config = ArtelConfig.model_validate(merged)
...
def resolve_model(config: ArtelConfig) -> tuple[str, str]:
