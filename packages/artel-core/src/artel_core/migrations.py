"""Migration system — versioned Artel bootstrap and first-run Artel state copy."""
...
import artel_core.config as config
...
def _record_global_migration(state: dict[str, Any], copied: list[str]) -> None:
    ...
    record = migrations.setdefault("artel_global_to_artel", {})
...
def migrate_project_state(project_dir: str | None) -> list[str]:
    """Copy legacy project-local Artel state into .artel for the given project."""
    if not project_dir:
        return []
...
@migration(2, "Copy legacy Artel global state into Artel config roots")
