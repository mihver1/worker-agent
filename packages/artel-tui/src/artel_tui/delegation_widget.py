...
class DelegationStatusWidget(Static):
    ...
    async def _load_runs(self) -> tuple[list[dict[str, Any]], str]:
        ...
        if not session_id:
            return [], ""
        if getattr(self._artel_app, "remote_url", ""):
            try:
                payload = await self._artel_app._remote_control().request(
                    "GET",
    ...
    def _current_session_id(self) -> str:
        if getattr(self._artel_app, "remote_url", ""):
            return str(getattr(self._artel_app, "_remote_session_id", "")).strip()
        session = getattr(self._artel_app, "_session", None)
